...
namespace Hackathon.EF_Core.Seed
{
    public static class DB_Seed
    {
        ...
        {
            new PointAttributeDbo() { Id = 1, Name = "Понижений вхід", Description = "наявність доступної вхідної групи до установи: понижений вхід (врівень із землею без влаштування ганку)."},
            new PointAttributeDbo() { Id = 2, Name = "Підйомні засоби", Description = "наявність доступної вхідної групи до установи: засоби для підйому (піднімальні платформи, вертикальні підйомники)"},
        ...
        private static List<PointDbo> PointsSeed(DbSet<PointAttributeDbo> pointAttributes)
        {
            ...
            new PointDbo()
            {
                ...
                Attributes = pointAttributes.Where(x => (new List<int>(){2}).Any(y => y == x.Id)).ToList()
            },
            new PointDbo()
            {
                Name = "Ощадбанк",
            ...
            new PointDbo()
            {
                ...
                Attributes = pointAttributes.Where(x => (new List<int>(){1}).Any(y => y == x.Id)).ToList()
            },
            new PointDbo()
            {
                Name = "ЕКО Маркет",
